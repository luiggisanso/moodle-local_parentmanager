...
class cohort_form extends \moodleform {
    public function definition() {
        ...
        $cohorts  = \local_parentmanager\helper::get_cohorts_menu();
        ...
        $mform->addElement('header', 'general', get_string('cohort_nav', 'local_parentmanager'));
        ...
        $cohort_options = ['' => get_string('choosedots')] + $cohorts;
        $mform->addElement('select', 'cohortid', get_string('select_cohort', 'local_parentmanager'), $cohort_options);
        $mform->addRule('cohortid', null, 'required', null, 'client');

        $this->add_action_buttons(false, get_string('load_cohort_btn', 'local_parentmanager'));
    }
}
